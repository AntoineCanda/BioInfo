...
public class Graines extends Windows {
	...
	/**
	 * Methode qui cree les sous sequences d'une sequence donnée à partir de la
	 * graine format
	 * 
	 * @param sequence
	 ...
	 */
	public ArrayList<String> traitementSubSequence(String sequence, String format) {
	 ...
	 */
	public ArrayList<ArrayList<String>> createSubSequenceGraines(String f1, String format) {
		String sequence = new String();
		ArrayList<ArrayList<String>> listResultat = new ArrayList<ArrayList<String>>(50);
	...
	public void compareKmersGraines(String f1, String f2, String graine) {
		...
		for (int i = 0; i < this.ssq1.size(); i++) {
			...
			ArrayList<ArrayList<String>> subRes = new ArrayList<ArrayList<String>>(50);
	...
	public void compareKmersGraines(String f1, String f2, String f3, String graine) {
	 ...
	 * @param n index of the sequence of the first file
	 * @param m index of the sequence of the second file
	 * @return the ratio : a double
	 */
	public double calculateRatio(int n, int m) {
		double ratio = (double) this.res.get(n).get(m).size() / this.ssq1.get(n).size();
		return ratio;
	}
	 ...
	 */
	public ArrayList<ArrayList<String>> calculateMapperWindows(int lw, int sw, String graine, double rk, String f1, String f2) {
		Windows windows = new Windows(f2);
		ArrayList<ArrayList<String>> listResWin = new ArrayList<ArrayList<String>>(50);
	...
	class ValueComparator implements Comparator {
	    ...
	    public int compare(Object o1, Object o2) {
	       if ((int) map.get(o1) >= (int) map.get(o2)) {
	          return -1;
	       } else {
	          return 1;
	       }
    ...
    sortByValue( Map<K, V> map )
{
	...
	public ArrayList<String> calculateMatches(String graine, String f1, String f2, int pos) {
	...
	public ArrayList<String> calculateMatchesBis(String graine, String f1, String f2, int pos) {
	...
	public ArrayList<String> calculateVJDiscover(String graine,String f1, String f2, String f3){
		Sequence seq = new Sequence(f1);
		...
		for(int i=0;i<seq.getListIdentifiant().size();i++){
			...
			TRGJ = TRGJ.concat(this.calculateMatchesBis(graine, f1, f3, i).get(0));
	...
	public void listKmers(String f, int taille) {
		...
		ArrayList<ArrayList<String>> sseq = this.createSubSequenceGraines(f, graine);
	...
	/**
	 * methode qui va afficher le ratio des kmers communs entre les differentes sequences
	 * Elle retourne la liste de liste des ratios
	 * @param f1
	 *            le fichier dont on extrait la premiere sequence
	 * @param f2
	 *            le fichier dont on extrait la seconde sequence
	 ...
	 */
	public ArrayList<ArrayList<Double>> ratioCommonKmers(String f1, String f2, int taille) {
		String graine = this.createGraine(taille);
		ArrayList<ArrayList<Double>> listListRatio = new ArrayList<ArrayList<Double>>(50);
		this.compareKmersGraines(f1, f2, graine);
		for(int i=0; i < this.ssq1.size(); i++){
			ArrayList<Double> listRatio = new ArrayList<Double>(50);
			for(int j=0; j < this.ssq2.size();j++){
				double ratio = calculateRatio(i,j);
				System.out.println("Ratio de la sequence " +i+" du fichier 1 et de la sequence "+j+" du fichier 2 : " + ratio);
				listRatio.add(ratio);
			}
			System.out.println("\n");
	 ...
	 *            le format des kmers
	 */
	public ArrayList<ArrayList<Double>> ratioCommonSpacedKmers(String f1, String f2, String graine) {
		ArrayList<ArrayList<Double>> listListRatio = new ArrayList<ArrayList<Double>>(50);
		this.compareKmersGraines(f1, f2, graine);
		for(int i=0; i < this.ssq1.size(); i++){
			ArrayList<Double> listRatio = new ArrayList<Double>(50);
	...
	/**
	 * la fonction affiche les fenetres dont le ratio de kmers de taille lk
	 * est superieur au ratio passe pour les 2 fichiers
	 * 
	 * @param lw
	 *            entier representant la longueur de la fenetre
	 * @param sw
	...
	public void mapperWindowsKmers(int lw, int sw, int lk, double rk, String f1, String f2) {
		...
		ArrayList<ArrayList<String>> resWin = this.calculateMapperWindows(lw, sw, graine, rk, f1, f2);
	...
	/**
	 * la fonction affiche les fenetres dont le ratio de kmers moyen obtenus
	 * via la graine est superieur au ratio passe pour les 2 fichiers
	 * 
	 * @param lw
	 *            entier representant la longueur de la fenetre
	 * @param sw
	...
	public void VJDiscover(String graine, String f1, String f2, String f3) {
		ArrayList<String> listRes = this.calculateVJDiscover(graine, f1, f2, f3);
		for (String res : listRes) {
			System.out.println(res);
		}
	}
